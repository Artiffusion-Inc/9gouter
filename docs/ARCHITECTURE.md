# 9Gouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

9Gouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[9Gouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.9gouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.9gouter/usage.json`, `~/.9gouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.9gouter/db.json`)
- usage stats: `~/.9gouter/usage.json`
- request log lines: `~/.9gouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[9Gouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.9gouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/9gouter && npm run build`
- Build Docker image: `cd /root/dev/9gouter && docker build -t 9gouter .`
