# 9router Go Backend Cutover Runbook
...
| Static binary build | `CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o 9router ./cmd/9router` | Binary produced |
| Cross-compile | `CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o 9router-arm64 ./cmd/9router` | Binary produced |
...
| Container image | `docker build -t 9router:cutover .` | Image builds and starts locally |
...
2. Build and push the cutover image:
   ...
   - `ghcr.io/artiffusion/9router:v0.6.0-cutover`
   - `ghcr.io/artiffusion/9router:latest`
   - `artiffusion/9router:v0.6.0-cutover`
   - `artiffusion/9router:latest`
...
2. Deploy the legacy JS backend image built from `legacy/js-backend`:
   ...
   docker build -t 9router-legacy:latest .
   # Or use the tag published before the cutover:
   # docker pull ghcr.io/artiffusion/9router:v0.5.XX-legacy
   docker run -d \
     --name 9router-legacy \
     -p 20128:20128 \
     -v 9router-data:/app/data \
     --env-file .env \
     9router-legacy:latest
   ```
...
| Resident set size (RSS) | `docker stats 9router` or `/proc/<pid>/status` | > 1 GB or monotonic growth |
