...
var configs = map[string]base.Config{
	...
	// kimi is the merged dual-auth provider (upstream v0.5.40, commit 68566f53):
	// OAuth = Kimi Code subscription (device-code flow, acquired out-of-band —
	// the dashboard device-code surface is not yet ported to Go), API key =
	// platform.moonshot / api.kimi.com. Transport is shared, so both auth
	// modes ride the same claude-format endpoint + kimiHeaders hook (the
	// X-Msh-* / device-id headers required by CLIProxyAPI parity). The legacy
	// `kimi-coding` id resolves to `kimi` via the alias map.
	"kimi": {
	...
	"kiro": {
		...
		Catalog: domain.ProviderCatalog{
			ID: "kiro", Alias: "kr", ServiceKinds: []string{"llm"},
			Models: []domain.Model{
...
var aliases = map[string]string{
	...
	"kimchi":        "kimchi",
	"kimi":          "kimi",
	// kimi-coding was merged into kimi as a dual-auth provider (upstream
	// v0.5.40); the legacy id resolves to the canonical `kimi`.
	"kimi-coding":       "kimi",
	"kiro":              "kiro",
	"minimax":           "minimax",
