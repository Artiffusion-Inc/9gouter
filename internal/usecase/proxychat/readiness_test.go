...
func TestReadiness_Disabled(t *testing.T) {
	...
	if r.Timeout != 0 || r.Reasons[0] != "disabled" {
...
func TestReadiness_BaseWhenNoBumps(t *testing.T) {
	...
	if r.Timeout != base {
		t.Fatalf("expected base %v, got %v", base, r.Timeout)
	}
...
func TestReadiness_LargeHistoryBump(t *testing.T) {
	...
	if r.Timeout != want {
		t.Fatalf("expected %v, got %v", want, r.Timeout)
	}
}
...
func TestReadiness_ClampToMax(t *testing.T) {
	...
	if r.Timeout != max {
		t.Fatalf("expected clamp at %v, got %v", max, r.Timeout)
	}
}
...
func TestReadiness_OnlyIncreases(t *testing.T) {
	...
	r := ResolveStreamReadinessTimeout(base, 15*time.Minute, "openai", "gpt-4", body)
	if r.Timeout < base {
		t.Fatalf("timeout decreased from %v to %v", base, r.Timeout)
	}
}
...
func TestReadiness_BumpsStack(t *testing.T) {
	...
	if r.Timeout != want {
		t.Fatalf("expected stacked %v, got %v", want, r.Timeout)
	}
}
