---
name: 9gouter-web-search
description: Web search via 9Gouter /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# 9Gouter — Web Search

Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/9gouter/refs/heads/master/skills/9gouter/SKILL.md for setup.
...
curl -X POST $NINEROUTER_URL/v1/search \
  ...
  -d '{"model":"tavily","query":"9Gouter open source","max_results":5}'
```
