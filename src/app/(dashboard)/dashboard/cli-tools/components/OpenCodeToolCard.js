...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("9gouter/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("9gouter/", ""));
    }
  }, [status]);
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_9gouter" : selectedApiKey);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.has9Gouter) return "not_configured";
    const url = status.config?.provider?.["9gouter"]?.options?.baseURL || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `9gouter/${activeModelToShow}`,
        agent: {
          explorer: {
            description: "Fast explorer subagent for codebase exploration",
            mode: "subagent",
            model: `9gouter/${effectiveSubagentModel}`
          }
        }
      }, null, 2),
    }];
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["9gouter"]?.options?.baseURL && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.has9Gouter} loading={restoring}>
